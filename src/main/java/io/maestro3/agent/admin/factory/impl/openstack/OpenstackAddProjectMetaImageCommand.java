...
@Component
public class OpenstackAddProjectMetaImageCommand extends AbstractAdminCommand<PlatformShapeMappingDto> {

    private final IOpenStackRegionRepository regionService;

    @Autowired
    public OpenstackAddProjectMetaImageCommand(IOpenStackRegionRepository regionService) {
        this.regionService = regionService;
    }

    @Override
    public PlatformShapeMappingDto getParams(String body, String... queryParams) {
        PlatformShapeMappingDto shapeMappingDto = JsonUtils.parseJson(body, PlatformShapeMappingDto.class);
        shapeMappingDto.setRegionAlias(queryParams[0]);
        return shapeMappingDto;
    }

    @Override
    public List<PlatformShapeMappingDto> buildRequests(SdkPrivateWizard wizard) {
        ...
        List<PlatformShapeMappingDto> requests = new ArrayList<>();
        ...
        for (SdkTableRowItem row : rows) {
            ...
            requests.add(new PlatformShapeMappingDto(imageNativeName, platform, regionName));
        }
        return requests;
    }

    @Override
    public PlatformShapeMappingDto buildRequest(SdkPrivateWizard wizard) {
        throw new UnsupportedOperationException("Single command generation is not supported");
    }

    @Override
    public AdminSdkResponse execute(PlatformShapeMappingDto shapeMappingDto) {
        String regionAlias = shapeMappingDto.getRegionAlias();
        ...
        EntityValidator.validate(shapeMappingDto);
        AdminProjectMeta adminProjectMeta = existingRegion.getAdminProjectMeta();
        if (adminProjectMeta == null) {
            throw new IllegalStateException("ERROR: Admin project meta is not configured for region. Received name " + regionAlias);
        }
        adminProjectMeta.getAvailablePublicImagesPlatformMapping().add(shapeMappingDto.toShapeMapping());
        regionService.save(existingRegion);
        return AdminSdkResponse.of("Platform shape mapping was configured for region " + regionAlias);
    }

    @Override
    public SdkAdminCommand prepareCommand(PlatformShapeMappingDto params) {
        String template = "m3admin private openstack add_project_meta_image --region_alias ${REGION_ALIAS}" + " --image ${IMAGE_NAME} --platform_type ${PLATFORM_TYPE}";
        Map<String, String> placeholders = new HashMap<>();
        placeholders.put("REGION_ALIAS", params.getRegionAlias());
        placeholders.put("IMAGE_NAME", params.getImageName());
        placeholders.put("PLATFORM_TYPE", params.getPlatformType());
    ...
    public AdminCommandType getType() {
        return AdminCommandType.OPEN_STACK_ADD_PROJECT_META_IMAGE;
    }
}
